...
CRYLLongAttackWeaponControl::CRYLLongAttackWeaponControl( CCharacterControl* lpObjControl ) 
{
	...
	memset( m_v3BombPos, 0, sizeof(D3DXVECTOR3) );
...
CRYLLongAttackWeaponControl::~CRYLLongAttackWeaponControl() 
{
}

//--------------------------------------------------------------------------------------------------------------------------
// CRYLLongAttackWeaponControl::UpdateControl()
static BOOL g_bRenderEnable = FALSE ;
HRESULT CRYLLongAttackWeaponControl::UpdateControl( BOOL bKeyAble, BOOL bEdge, int MouseX,int MouseY ) 
{
	...
	{                                                   
        ...
        if( !m_bKeyPress )
		{
			...
			{   
                float fUpdate = CFrameTimer::GetUpdateTimer( GRYLTimer.GetSceneManagerTimerID() );
               ...
               /* if ( KeyPressed( 'M' ) )
				{
					vector3 vecToward ;
					vecToward.x = cosf( m_fRealTireAngle + FLOAT_PHI / 2.0f ) ;
					vecToward.z = -sinf( m_fRealTireAngle + FLOAT_PHI / 2.0f ) ;	
					vecToward.y = 0.0f ;
					vecToward.Normalize() ;

					//vecChrToward = vecToward ;
					//vecChrToward.Normalize() ;

					g_bRenderEnable = TRUE ;
					vector3 v3Start, v3Dest ;
					v3Start.x = pSelfModel->GetPosition().x ;
					v3Start.y = pSelfModel->GetPosition().y ;
					v3Start.z = pSelfModel->GetPosition().z ;
					
					v3Dest = v3Start + ( vecToward * 10000 ) ; //vecChrToward * 10.0f ;//v3Start * 0.5 ;
					float fTempY = CSceneManager::m_HeightField.GetHeight( v3Dest ) ;	
					v3Dest.y = fTempY ;
					CreateBombLine( v3Start, v3Dest ) ;
					
					if( FAILED( CSceneManager::GetDevice()->CreateVertexBuffer( 100 * sizeof( SDiffuseVertex ),
																				0, 
																				FVF_VERTEX, 
																				D3DPOOL_MANAGED,
																				&m_pVertexBuffer ) ) )
					...
					VOID* pVertices ;
					if ( FAILED( m_pVertexBuffer->Lock( 0, sizeof( m_DiffuseVertex ), ( BYTE** )&pVertices, 0 ) ) )
					{
						return E_FAIL ;
					}
					memcpy( pVertices, m_DiffuseVertex, sizeof( SDiffuseVertex ) * 100 ) ;
					...
					test_script->GetScriptBinData( "FireBolt2.esf" ) ;
					test_script->SetStartPos( m_DiffuseVertex[ 0 ].Position.x,
											  m_DiffuseVertex[ 0 ].Position.y,
											  m_DiffuseVertex[ 0 ].Position.z ) ;
					test_script->SetEndPos( m_DiffuseVertex[ 99 ].Position.x, 
						                    m_DiffuseVertex[ 99 ].Position.y, 
											m_DiffuseVertex[ 99 ].Position.z ) ;
					test_script->SetChr( pSelfModel, pSelfModel ) ;
            ...
            if( !m_lpObjControl->m_bSummonCmd )
			{
                if( !pSelfCreature->GetEffect()->GetChantStatus( Skill::SpellID::Stun ) )
				{
                    ...
                    if ( bBombEnable && 100.0f < pSelfModel->GetMotionCurrentFrame( 0 ) )
					{
						vector3 vecToward ;
						vecToward.x = cosf( m_fRealTireAngle + FLOAT_PHI / 2.0f ) ;
						vecToward.z = -sinf( m_fRealTireAngle + FLOAT_PHI / 2.0f ) ;	
...
//--------------------------------------------------------------------------------------------------------------------------
VOID		CRYLLongAttackWeaponControl::SetModifySkileton() 
{
    CZ3DGeneralChrModel* pSiegeModel = m_lpSiegeModel->GetCharModel();
    if( !pSiegeModel )
        return;

	m_lpLeftTireObject  = pSiegeModel->GetSkeletonByIndex( 25 ) ;
	m_lpRightTireObject = pSiegeModel->GetSkeletonByIndex( 23 ) ;
...
//--------------------------------------------------------------------------------------------------------------------------
// CRYLLongAttackWeaponControl::SetSiegeModel()
VOID		CRYLLongAttackWeaponControl::SetSiegeModel( unsigned long dwChrID, const char *strName, vector3 vecChrPos, float fDirection ) 
{
    m_lpSiegeModel = RYLCreatureManager::Instance()->AddSiegeCreature( m_lpSiegeModel, dwChrID, strName, vecChrPos, fDirection, "LongSeigeWeapon" );
}

//--------------------------------------------------------------------------------------------------------------------------
// CRYLLongAttackWeaponControl::CreateBombLine()
HRESULT		CRYLLongAttackWeaponControl::CreateBombLine( vector3& v3Start, vector3& v3Dest) 
{
	HRESULT hr = S_OK ;
	
	v3Start.y += 1100.0f ;

	FLOAT		  fStartHeight = v3Start.y + 2000.0f ;
	FLOAT		  fDestHeight  = v3Dest.y + 2000.0f ;

	D3DXVECTOR3		v3BowSide[ 4 ] ;
	v3BowSide[ 0 ].x = v3Start.x ;	v3BowSide[ 0 ].y = v3Start.y ;		v3BowSide[ 0 ].z = v3Start.z ;
	v3BowSide[ 1 ].x = v3Start.x ;	v3BowSide[ 1 ].y = fStartHeight ;	v3BowSide[ 1 ].z = v3Start.z ;
	v3BowSide[ 2 ].x = v3Dest.x ;	v3BowSide[ 2 ].y = fDestHeight ;	v3BowSide[ 2 ].z = v3Dest.z ;
	v3BowSide[ 3 ].x = v3Dest.x ;	v3BowSide[ 3 ].y = v3Dest.y ;		v3BowSide[ 3 ].z = v3Dest.z ;
	
	D3DXVECTOR3		v3Result ;
	FLOAT			fTemp ;
	INT				N = 3, iBombNum = 100 ;

	for ( INT u = 0 ; u <= iBombNum ; u ++ )
	{
		v3Result = D3DXVECTOR3( 0.0f, 0.0f, 0.0f ) ;
		for ( INT i = 0 ; i < N + 1 ; i ++ )
		{
			fTemp = CalcBi_n( N, i, ( u * 0.01f ) ) ;
			v3Result = v3Result + fTemp * v3BowSide[ i ] ;
		}
		m_DiffuseVertex[ u ].Position			= v3Result ;
		m_DiffuseVertex[ u ].Color				= 0xffffffff ;
	}
...
//--------------------------------------------------------------------------------------------------------------------------
// CRYLLongAttackWeaponControl::CalcBi_n()
double		CRYLLongAttackWeaponControl::CalcBi_n( INT n, INT i, DOUBLE u ) 
{
	INT			j ;
...
//--------------------------------------------------------------------------------------------------------------------------
// CRYLLongAttackWeaponControl::SetCamera()
VOID CRYLLongAttackWeaponControl::SetCamera( int dx, int dy, int dz )
{	
    RYLCreature* pSelfCreature = RYLCreatureManager::Instance()->GetSelfCreature();
    if( !pSelfCreature )
        return;
...
void		CRYLLongAttackWeaponControl::Render( LPDIRECT3DDEVICE8 pd3dDevice ) 
{
}

//--------------------------------------------------------------------------------------------------------------------------
// CRYLLongAttackWeaponControl::MakeAttackInfo()
bool CRYLLongAttackWeaponControl::MakeAttackInfo( unsigned short wSkill, unsigned char cSkillLock, unsigned char cSkillLevel, 
...
//--------------------------------------------------------------------------------------------------------------------------
// CRYLLongAttackWeaponControl::MakeSkillAttackInfo()
bool CRYLLongAttackWeaponControl::MakeSkillAttackInfo( unsigned long dwTargetID, unsigned short wSkill, 
