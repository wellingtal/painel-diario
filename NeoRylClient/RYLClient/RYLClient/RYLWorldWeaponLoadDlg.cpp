...
CRYLWorldWeaponLoadDlg*	g_pRYLWorldWeaponLoadDlg = NULL ;

CRYLWorldWeaponLoadDlg::CRYLWorldWeaponLoadDlg() 
{
	RECT rt ;
...
VOID	CRYLWorldWeaponLoadDlg::InitCtrl() 
{
	...
	m_pArmsCreatMenuLabel->SetCaption( CRYLStringTable::m_strString[ 3191 ] ) ;
	...
	m_pArmsCreatExpla1Label->SetCaption( CRYLStringTable::m_strString[ 3192 ] ) ;
	...
	m_pArmsCreatExpla3Label->SetLeft( 81 ) ;
	...
	m_pArmsCreatExpla3Label->SetCaption( CRYLStringTable::m_strString[ 3183 ] ) ;
	AttachGMUIControl( m_pArmsCreatExpla3Label ) ;

	// 병기 생성 설명 4
	m_pArmsCreatExpla4Label = new CRYLLabel( this ) ;
	m_pArmsCreatExpla4Label->SetLeft( 81 ) ;
	...
	m_pArmsCreatExpla4Label->SetCaption( CRYLStringTable::m_strString[ 3184 ] ) ;
	...
	m_pOKButton->SetInstance( RYLWORLDWEAPONLOADDLG::OK_BUTTON ) ;
	...
	m_pCancelButton->SetInstance( RYLWORLDWEAPONLOADDLG::CANCEL_BUTTON ) ;
...
VOID	CRYLWorldWeaponLoadDlg::FinalCtrl() 
{
...
VOID	CRYLWorldWeaponLoadDlg::InitValue() 
{
}

VOID	CRYLWorldWeaponLoadDlg::Show( CGMUIObject* Sender ) 
{
	CHAR strString[MAX_PATH];
	...
	if ( pCamp )
	{
		...
		if( pTempCreature && pTempCreature->GetSubState() == Siege::WEAPON_EMPTY )
		{
...
VOID	CRYLWorldWeaponLoadDlg::Hide( CGMUIObject* Sender ) 
{
	//CRYLGameData::Instance()->m_lpStartKit = NULL ;
}

VOID	CRYLWorldWeaponLoadDlg::GMUIPaint( INT xx, INT yy ) 
{
...
HRESULT		CRYLWorldWeaponLoadDlg::Render( LPDIRECT3DDEVICE8 lpD3DDevice )  
{
	return S_OK ;
}

HRESULT		CRYLWorldWeaponLoadDlg::FinalRender( LPDIRECT3DDEVICE8 lpD3DDevice ) 
{
	return S_OK ;
}

HRESULT		CRYLWorldWeaponLoadDlg::Update( BOOL &bClick, BOOL &bEdge ) 
{
	if (FALSE == GetVisible())
	{
		return S_OK;
...
VOID	CRYLWorldWeaponLoadDlg::DlgClick( CGMUIObject* Sender, INT X, INT Y ) 
{
	if ( !g_pRYLWorldWeaponLoadDlg )
		return ;

	switch( Sender->GetInstance() )
	{
		case RYLWORLDWEAPONLOADDLG::OK_BUTTON :
			{
				CRYLCamp* pCamp = CRYLCampManager::Instance()->FindCamp( g_pRYLWorldWeaponLoadDlg->m_dwCampID );
				if ( pCamp )
				{
					if( g_pRYLWorldWeaponLoadDlg->m_nMaterialNeed == 0 )
					{
						CRYLNetworkData* pNetwork = CRYLNetworkData::Instance() ;
						SendPacket::CharCampCmd( g_GameSession, 
												pNetwork->m_dwMyChrID, 
												g_pRYLWorldWeaponLoadDlg->m_dwCampID, 
												0, 
												0,
												PktCampCmd::WORLDWEAPON_CHARGE ) ;
							

						g_pRYLWorldWeaponLoadDlg->SetVisible( FALSE ) ;
					}
					else if ( pCamp->GetMaterial() )
					{
						if ( g_pRYLWorldWeaponLoadDlg->m_nMaterialNeed <= pCamp->GetMaterial()->GetNowDurability())
						{
							CRYLNetworkData* pNetwork = CRYLNetworkData::Instance() ;
							SendPacket::CharCampCmd( g_GameSession, 
													pNetwork->m_dwMyChrID, 
			...
			break ;
		case RYLWORLDWEAPONLOADDLG::CANCEL_BUTTON :
			{
				g_pRYLWorldWeaponLoadDlg->SetVisible( FALSE ) ;
			}
			break ;
